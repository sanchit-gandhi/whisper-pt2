...
""" PyTorch Whisper model with bitsandbytes quantization and PT 2.0 scaled dot product attention (sdpa)."""
...
# Copied from transformers.models.whisper.modeling_whisper.WhisperAttention with nn.Linear->bnb.nn.Linear8bitLt
class WhisperAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = bnb.nn.Linear8bitLt(embed_dim, embed_dim, bias=False)
    ...
    # Copied from modeling_whisper_attn.WhisperAttention.forward
    def forward(
        self,
...
# Copied from transformers.models.whisper.modeling_whisper.WhisperEncoderLayer with nn.Linear->bnb.nn.Linear8bitLt
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        self.fc1 = bnb.nn.Linear8bitLt(self.embed_dim, config.encoder_ffn_dim)
        self.fc2 = bnb.nn.Linear8bitLt(config.encoder_ffn_dim, self.embed_dim)
...
# Copied from transformers.models.whisper.modeling_whisper.WhisperDecoderLayer with nn.Linear->bnb.nn.Linear8bitLt
class WhisperDecoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig):
        ...
        self.fc1 = bnb.nn.Linear8bitLt(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = bnb.nn.Linear8bitLt(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = nn.LayerNorm(self.embed_dim)
...
# Copied from transformers.models.whisper.modeling_whisper.WhisperPreTrainedModel with nn.Linear->bnb.nn.Linear8bitLt
class WhisperPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (bnb.nn.Linear8bitLt, nn.Conv1d)):
...
# Copied from transformers.models.whisper.modeling_whisper.WhisperForConditionalGeneration with nn.Linear->bnb.nn.Linear8bitLt
class WhisperBnbFlashAttnForConditionalGeneration(WhisperPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.proj_out = bnb.nn.Linear8bitLt(config.d_model, config.vocab_size, bias=False)
